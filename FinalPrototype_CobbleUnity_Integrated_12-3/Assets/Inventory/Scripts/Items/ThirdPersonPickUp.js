...
private var inventoryObject: GameObject;

@script AddComponentMenu ("Inventory/Items/Third Person Pick Up")
...
function Update ()
{
	if (thePlayer != null)
	{
		...
		if (Input.GetButton("PickUp") && canPickUp == true)
		{
			theItem.PickUpItem();
			canPickUp = false;
		}
...
function OnTriggerEnter(other: Collider)
{
	...
	//add name of interactive object to inventory object variable as long as collider is not a trigger
	if(!other.isTrigger && (other.gameObject.tag == "InvObject"))
	{
		//add object to array
		inventoryObject = other.gameObject;
		Debug.Log(inventoryObject.gameObject.name);
		
	}
}

//Deletes Inventory Objects from array that leave trigger area
function OnTriggerExit(other: Collider)
{
	//If object is in the array remove it from the arrayList
	if(!other.isTrigger && (other.tag == "InvObject"))
	{
